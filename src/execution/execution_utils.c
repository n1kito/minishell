...
void	close_pipes(t_master *master)
{
	...
	{
		if ((master->pipes[i][0] >= 0 && close(master->pipes[i][0]) == -1)
			|| (master->pipes[i][1] >= 0 && close(master->pipes[i][1]) == -1))
...
void	close_files(t_master *master, int i)
{
	...
	if (master->commands[i] && master->commands[i]->heredoc_path
